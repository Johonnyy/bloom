...
def test_the_expected_tools_and_resources_are_registered():
    ...
    ``build_agent`` and ``edit_agent`` are the additions that earn their place,
    because they are the exception to that rule: "make me a config" and "change that
    config" cannot themselves be configs.
    """
    mcp = mcp_module.build_server(_settings())
    assert {t.name for t in mcp.tool_policies()} == {
        "run_task",
        "list_agents",
        "build_agent",
        "edit_agent",
...
def test_query_tools_are_marked_read_only():
    ...
    # Not read-only: they write configuration and spend money.
    assert policies["build_agent"].read_only is False
    assert policies["edit_agent"].read_only is False
